...
void dpx_init_d3d12() {
    ...
    for (UINT i = 0; factory->EnumAdapters1(i, &adapter) != DXGI_ERROR_NOT_FOUND; ++i) {
        ...
        if (SUCCEEDED(D3D12CreateDevice(adapter.Get(), D3D_FEATURE_LEVEL_12_0, IID_PPV_ARGS(&g_device)))) {
    ...
    g_device->CreateCommandQueue(&qd, IID_PPV_ARGS(&g_compute_queue));
    ...
    if (SUCCEEDED(D3D12SerializeRootSignature(&rsd, D3D_ROOT_SIGNATURE_VERSION_1, &sig, &err))) {
        g_device->CreateRootSignature(0, sig->GetBufferPointer(), sig->GetBufferSize(), IID_PPV_ARGS(&g_root));
    }
    ...
    if (!gemv_dxil.empty()) {
        ...
        g_device->CreateComputePipelineState(&pd, IID_PPV_ARGS(&g_pso_gemv));
    }
    if (!tiled_dxil.empty()) {
        D3D12_COMPUTE_PIPELINE_STATE_DESC pd = {};
        pd.pRootSignature = g_root.Get();
        ...
        g_device->CreateComputePipelineState(&pd, IID_PPV_ARGS(&g_pso_tiled));
    }
}
