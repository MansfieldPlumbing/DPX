...
// PRE-COMPILED DISPATCH PIPELINES
// Mechanistic mapping: replaces C# Dp.Dispatch string routing.
struct DispatchCacheKey {
    ...
    bool operator==(const DispatchCacheKey& o) const {
        return M == o.M && N == o.N && K == o.K && block_size == o.block_size && mode == o.mode;
    }
};

struct KeyHash {
    size_t operator()(const DispatchCacheKey& k) const { return k.M ^ k.N ^ k.K ^ k.mode; }
};
...
// Submit without blocking the CPU thread
