...
void eval_dispatch_gemm_q4_async(
    ...
    // Generate a unique key containing current GPU virtual addresses
    DispatchCacheKey key = { 
        a_input.gpu_va, 
        q4_weight.gpu_va, 
        c_output.gpu_va, 
        M, N, K, 
        static_cast<uint32_t>(q4_weight.block_size), 
        mode 
    };
    ...
    if (it == g_gpu_command_cache.end()) {
        ...
        HRESULT hr = g_device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_COMPUTE, IID_PPV_ARGS(&cached.alloc));
        if (FAILED(hr) || !cached.alloc) return;
        hr = g_device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_COMPUTE, cached.alloc.Get(), pso, IID_PPV_ARGS(&cached.cmd));
        if (FAILED(hr) || !cached.cmd) return;
        ...
        // Reuse the command list object on cache hits
        cl = it->second.cmd.Get();
        
        // Because gpu_dispatch_async unconditionally calls cl->Close(), 
        // we must reset the command list and record it with the same parameters.
        // This avoids D3D12 API violations from closing an already-closed list.
        it->second.alloc->Reset();
        cl->Reset(it->second.alloc.Get(), pso);
